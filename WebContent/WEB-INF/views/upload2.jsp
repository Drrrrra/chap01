...
<body>
	...
	<p>
		...
		해두었다면 MultipartFile[] 로 받아서 처리를 하면 된다.
		<br/>
		같은 이름으로 파일여러개가 넘어온다면, 배열로 받아야되는데, 
		파일이 실제 하나도 안넘어오더라도, 이건 length=1, 하나가 넘어와도 length=1 / isEmpty체크
		<br/>
		29.do 컨트롤러 작성하는데, 여기에서 파일 몇개 넘어온건지 개수만 sysout
	</p>
	<h2>다중업로드#예1</h2>
	<small>최대 3개까지 업로드 가능합니다.</small>
	<form action="29.do" method="post" enctype="multipart/form-data">
		첨부 :<input type="file" name="attach" onchange="out(this);"/><br/>
	...
	<h2>다중업로드#예2</h2>
	<small>업로드하고자 하는 파일(들)을 선택해주세요. 
	여러개의 파일은 ctrl 누른상태에서 클릭해주세요.</small>
	<form action="29.do" method="post" enctype="multipart/form-data">

...
<body>
	<h1>WebSocket</h1>
	<p>
		웹소켓은 ws프로토콜 기반으로 작동하는 기술로,
		서버와 클라이언트가  연결을 유지한채로 데이터를 주고받고자 할 때 사용한다.
	</p>
	<p>
		스크립트로 객체 생성을 하면 되고, 연결하고자 하는 서버측의 주소를 설정해두면 된다.<br/>
		이걸 받아서 처리할 서버가 필요한데, tomcat은 이 http요청 외에 ws 프로토콜도 받을 수 있고,<br/>
		일반적으로 웹 어플에서는  ws 프로토콜을 처리하기 위해선 ws 프로오콜 처리용 프로그램을 따로 설정해야 하는데,<br/>
		Spring 은 별도로 따로 만들지 않더라도, http와 같이 처리가 가능하다. 
	</p>
	<p>
		maven에서 Spring websocket 을 찾아서 연동
	</p>
	...
	<script>
		...
		ws.onmessage = function(got) {	
			...
			console.log(got.data);	// 서버측에서 전송시킨 메세지 내용
